...
def read_save():
    ...
    try:
        ...
        file_save2.close()
        for item in info:
            tree.insert('', 0, text='{}'.format(item['text']), values=(item['values']))
    except IOError as e:
        print "I/O error({0}): {1} - You had never saved your settings".format(e.errno, e.strerror)
    ...
    while result == None and i < 10**5:
        try:
            ...
            if f_size > 0:
                ...
                print "its x: ", x
                for item in x:
                        tree.insert('', 0, text='{}'.format(item['info_text']),
...
def sendFile():
    ...
    files = {'file': (root.filename+".exe", open(root.filename, 'rb'))}
...
def sendFile2():
    ...
    if (json_response["positives"] / json_response["total"]) * 100 > 50:
        ...
        tree.insert('', 0, text='malware detected! : {}'.format(root.filename), values=())
    else:
        print "clean!"
        tree.insert('', 0, text='clean! : {}'.format(root.filename), values=())
    for line in json_response:
...
menu_file.add_command(label="Scan warnings", command=get_warnings)
...
Send_Button2 = ttk.Button(f2, text="Send file to get report", command=sendFile2)
